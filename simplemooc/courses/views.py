...
from .models import Course, Enrollment, Lessons, Material
...
def enrollment(request, slug):
    ...
    if created:
        ...
        messages.info(request,'Você ja é inscrito no curso')
...
def show_announcements(request, slug, pk):
    ...
    announcement=get_object_or_404(course.announcements.all(), id=pk)
    if form.is_valid():
        ...
        comment.save()
        form=CommentForm()
        messages.success(request, 'Seu comentário foi adcionado com sucesso')
...
def show_lesson(request, slug, pk):
    course=request.course
    lesson=get_object_or_404(Lessons, course=course, id=pk)
    if not request.user.is_staff and not lesson.is_available():
        messages.error(request, 'Essa aula não está disponível')
...
def material(request, slug, pk):
    course=request.course
    material=get_object_or_404(Material, id=pk, lesson__course=course)
    ...
    if not material.is_embedded():
        return redirect(material.files.url)
